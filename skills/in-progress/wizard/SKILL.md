...
- `chmod +x <script>`.
- Don't run it end-to-end yourself — it opens browsers and blocks on human input. Trace it statically instead: every value from step`.env`is captured and lands where step`.env`said, and every `set_secret` name exactly matches a `secrets.*` reference in CI.
